...
export default function Home() {
  ...
  const filteredCatalog = catalog.filter((item) => { const matchesCategory = categoryFilter === "Toutes" || item.category === categoryFilter; const query = searchQuery.trim().toLowerCase(); return matchesCategory && (!query || `${item.title} ${item.category} ${item.description}`.toLowerCase().includes(query)); });
  ...
  return <main className="public-app">
    ...
    <section className="catalog-section" id="catalog"><div className="section-top"><div><div className="public-kicker">02 bis · Prestations disponibles</div><h2>Des offres<br /><em>à demander.</em></h2></div><p>Les responsables de domaine publient ici leurs prestations. Choisissez une offre pour demander un devis personnalisé.</p></div><div className="catalog-toolbar"><label className="catalog-search"><Search size={16} /><span className="sr-only">Rechercher une prestation</span><input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Rechercher une prestation..." /></label><div className="category-filters" aria-label="Filtrer par catégorie">{categories.map((category) => <button type="button" key={category} className={categoryFilter === category ? "active" : ""} onClick={() => setCategoryFilter(category)}>{category}</button>)}</div></div><div className="catalog-grid">{filteredCatalog.map((service) => <article className="catalog-card" key={service.id}><div className="catalog-card-top"><span>{service.category}</span><span className="catalog-status">Publié</span></div><h3>{service.title}</h3><p>{service.description}</p><div className="catalog-card-bottom"><strong>{service.priceLabel}</strong><button onClick={() => requestQuote(service)}>Demander un devis <ArrowRight size={15} /></button></div><small>Publié par {service.publisher}</small></article>)}</div>{filteredCatalog.length === 0 && <div className="catalog-empty">Aucune prestation ne correspond à votre recherche. Essayez une autre catégorie ou un autre terme.</div>}</section>
