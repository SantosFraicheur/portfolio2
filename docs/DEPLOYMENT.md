...
Le projet est une application React/Vite avec serveur Express, tRPC, authentification Manus OAuth, base MySQL/TiDB via Drizzle et stockage objet sécurisé pour les images.
...
`DATABASE_URL` reste strictement côté serveur. En production, utilisez une URL MySQL/TiDB fournie par l’hébergeur avec TLS activé, un compte applicatif à privilèges limités et une base séparée de la base de développement. Ne placez jamais `DATABASE_URL`, `JWT_SECRET` ou une clé de stockage dans une variable `VITE_*`, car ces variables peuvent être intégrées au bundle client.
